...
library(ggplot2)
ggplot(Dataset, aes(x=SalePrice))+geom_histogram(aes(y=..density..),colour="black",fill="white")+ # Buat base histogramnya
  geom_density(alpha=.2,fill="#0000FF")+ # Tambahkan densitas dengan warna merah muda
  labs(title="Distribusi Data Sale Price",x="SalePrice",y="Count of SalePrice")+ # Tambahkan judul dan label
  theme_minimal()+
  theme(plot.title = element_text(hjust = 0.5))+ #Pusatkan judul
  scale_x_continuous(breaks = seq(min(Dataset$SalePrice),
                                  max(Dataset$SalePrice),by=((max(Dataset$SalePrice)-
                                                                min(Dataset$SalePrice))/5)))
# Delete outliers
Dataset <- subset(Dataset, SalePrice <=300000)
boxplot(Dataset$SalePrice)

# Buat histogram distribusi setelah filtering
ggplot(Dataset, aes(x=SalePrice))+geom_histogram(aes(y=..density..),colour="black",fill="white")+ # Buat base histogramnya
  geom_density(alpha=.2,fill="#0000FF")+ # Tambahkan densitas dengan warna merah muda
  labs(title="Distribusi Data Sale Price",x="SalePrice",y="Count of SalePrice")+ # Tambahkan judul dan label
  theme_minimal()+
...
fviz_nbclust(Dataset, kmeans, method = "wss") # metode elbow

fviz_nbclust(Dataset, kmeans, method = "silhouette") # metode silhouette
set.seed(123)
...
final <- kmeans(Dataset, 6, nstart = 25)
print(final)

fviz_cluster(final, data = Dataset)
...
over <- ovun.sample(SalePrice~., data = train, method = "over", N=594*2)$data
...
##UnderSampling
table(train$SalePrice)
under <- ovun.sample(SalePrice~., data = train, method = "under", N=492*2)$data
